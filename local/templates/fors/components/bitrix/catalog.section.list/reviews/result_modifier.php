...
if(!empty($arResult['SECTIONS'])){
	...
	foreach($arResult["SECTIONS"] as $sect){
		$arSelect = Array("ID", "NAME", "IBLOCK_ID", "PREVIEW_PICTURE", "PREVIEW_TEXT", "DATE_ACTIVE_FROM", "PROPERTY_*");
		$arFilter = Array("IBLOCK_ID"=>$arParams["IBLOCK_ID"], "IBLOCK_SECTION_ID"=>$sect["ID"], "ACTIVE"=>"Y");
		$res = CIBlockElement::GetList(Array("PROPERTY_PRICE"=>"ASC"), $arFilter, false, Array("nTopCount"=>4), $arSelect);
		while($ob = $res->GetNextElement())
		{
			$fields = $ob->GetFields();
			$properties = $ob->GetProperties();
			$fields["PROPERTIES"] = $properties;
			$arResult["ITEMS"][$sect["ID"]][] = $fields;
		}
	}
}
?>