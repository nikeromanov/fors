...
(function () {
  ...
  function initDistrictTabs() {
    ...
    groups.forEach((group) => {
      ...
      const defaultMapHtml = mapContainer ? mapContainer.innerHTML : '';
      ...
      const renderMapHtml = (html) => {
        if (!mapContainer) return;
        const nextHtml = html || defaultMapHtml || '';
        if (mapContainer.innerHTML !== nextHtml) {
          mapContainer.innerHTML = nextHtml;
        }
      };
